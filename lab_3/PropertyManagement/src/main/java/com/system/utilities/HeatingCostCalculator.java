...
public class HeatingCostCalculator implements MeterReadingUtilityCalculator {
    private Map<Property, List<HeatingMeterReading>> heatingReadings = new HashMap<>();
    private Tariff tariff;

    public HeatingCostCalculator(Tariff tariff) {
        this.tariff = tariff;
    }

    @Override
    public void addReading(Property property, Object reading) {
        if (reading instanceof HeatingMeterReading) {
            heatingReadings.computeIfAbsent(property, k -> new ArrayList<>())
                    .add((HeatingMeterReading) reading);
        } else {
            throw new UtilityServiceException("Reading must be of type HeatingMeterReading");
        }
    }

    public List<HeatingMeterReading> getHeatingReadings(Property property) {
        return heatingReadings.getOrDefault(property, new ArrayList<>());
    }

    public double calculateCost(HeatingMeterReading reading) {
        if (tariff == null) {
            throw new UtilityServiceException("Tariff is not set");
        }
        return reading.getConsumption() * tariff.getHeatingRate();
    }
    ...
    public double calculateTotalCost(Property property, LocalDate startDate, LocalDate endDate) {
        ...
        return getHeatingReadings(property).stream()
